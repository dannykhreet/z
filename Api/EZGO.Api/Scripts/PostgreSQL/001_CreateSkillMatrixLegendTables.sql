...
BEGIN
    RETURN QUERY SELECT c.id, c.company_id, c.version, c.created_at, c.updated_at, c.created_by, c.updated_by
    FROM skill_matrix_legend_configuration c WHERE c.company_id = _company_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY SELECT i.id, i.configuration_id, i.skill_level_id, i.skill_type, i.label, i.description, i.icon_color, i.background_color, i.sort_order, i.score_value, i.icon_class, i.is_default, i.created_at, i.updated_at
    FROM skill_matrix_legend_item i WHERE i.configuration_id = _configuration_id AND i.skill_type = _skill_type ORDER BY i.sort_order;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    VALUES (_company_id, _version, NOW(), _created_by) RETURNING id INTO v_id;
    RETURN v_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    VALUES (_configuration_id, _skill_level_id, _skill_type, _label, _description, _icon_color, _background_color, _sort_order, _score_value, _icon_class, _is_default, NOW()) RETURNING id INTO v_id;
    RETURN v_id;
END;
$$ LANGUAGE plpgsql;
...
RETURNS BOOLEAN AS $$
BEGIN
    UPDATE skill_matrix_legend_item i SET label = _label, description = _description, icon_color = _icon_color, background_color = _background_color, sort_order = _sort_order, score_value = _score_value, icon_class = _icon_class, is_default = FALSE, updated_at = NOW()
    FROM skill_matrix_legend_configuration c WHERE i.configuration_id = c.id AND c.company_id = _company_id AND i.skill_level_id = _skill_level_id AND i.skill_type = _skill_type;
    RETURN FOUND;
END;
$$ LANGUAGE plpgsql;
