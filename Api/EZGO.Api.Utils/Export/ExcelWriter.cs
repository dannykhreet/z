            ...
            if (source != null && source.Tables != null && source.Tables.Count > 0)
            {
                ...
                foreach (DataTable dt in source.Tables)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        for (var i = 0; i < numberOfColumns; i++)
                        {
                            XLCellValue excelCellValue = default;
                            try
                            {
                                excelCellValue = XLCellValue.FromObject(dr[i]);
                            }
                            catch (ArgumentOutOfRangeException exception)
                            {
                                excelCellValue = XLCellValue.FromObject("Data too large for cell; not supported by excel");
                            }
