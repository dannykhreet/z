...
namespace EZGO.Maui.Platforms.iOS.Services
{
    public class CachingService : ICachingService
    {
        ...
        public void ClearCache(string request = null)
        {
            try
            {
                string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), cacheFolder);
        ...
        public async Task AlterCachedRequestListAsync<T>(string request, Action<T> alteringFunction, Func<T, bool> predicate = null)
        {
            try
            {
                ...
                var data = await httpResponse.Content.ReadAsJsonAsync<IEnumerable<T>>();
        ...
        public void ClearOutOfDateCache()
        {
            ...
            if (Directory.Exists(folder))
            {
                ...
                foreach (string filename in filenames)
                {
                    ...
                    //TODO Check how it will react to different dates
                    //LocalDateTime datetime = DateTimeHelper.FromFileTimeUtc(fileTime);
        ...
        private static string GetFilenameToRetrieveRequest(string request)
        {
            ...
            if (Directory.Exists(folder))
            {
                long current = DateTime.UtcNow.ToFileTimeUtc();
        ...
        {
            var fileTime = DateTime.UtcNow.Add(duration).ToFileTimeUtc();

            string filename = $"{fileTime}-{request.RemoveSpecialCharacters().Replace(" ", "_")}.req";
            Debug.WriteLine($"Cache Filename: {filename}");
